...
# ASE & Pymatgen conversions
from pymatgen.io.ase import AseAtomsAdaptor
from ase.optimize import BFGS

# MACE
# Wrap in try-except to prevent import errors if mace isn't installed 
# on the machine defining the class (though it must be on workers)
try:
...
class mlip_mace(object):

    def __init__(self, energy_params):
        """
        energy_params: dictionary of all the parameters from your YAML file.

        Expected YAML keys for this class:
        ```yaml
        main_path: /path/to/project
        mace_model: mace-mp-0  # or medium, large, mace-matpes-pbe-0
        mace_device: cuda      # or cpu
        fmax: 0.01             # force convergence criteria
        max_steps: 500         # max relaxation steps
        element_syms: ...      # needed for chemical potentials
        mu: ...                # needed for chemical potentials
        ```
        """
        self.main_path = energy_params.get('main_path', '.')
        
        # MACE specific parameters
        # Default to mace-mp-0 (medium) if not provided
        self.model_type = energy_params.get('mace_model', 'mace_mp') 
        self.device = energy_params.get('mace_device', 'cuda')
        ...
        # Chemical Potentials (Identical logic to lammps_code for consistency)
        self.sym_mu_dict = {}
        if 'element_syms' in energy_params and 'mu' in energy_params:
            for key, value in energy_params['element_syms'].items():
                self.sym_mu_dict[value] = energy_params['mu'][key]
        else:
            warnings.warn("Chemical potentials ('mu' or 'element_syms') missing from inputs.")

    def prep_job_folder(self, model, reg_id):
        """
        Creates the folder structure. 
        Unlike LAMMPS, we don't need to copy potential files or 'in.min', 
        but we still need the folder for logs and saving the POSCARs.
        """
        main_path = self.main_path
        
        # Retry logic for creating directories (ProcessPool safety)
        while True:
        ...
        os.makedirs(relax_path, exist_ok=True)
        
        self.relax_path = relax_path
        model.relax_path = relax_path
        
        # Write unrelaxed structure for record keeping
        new_poscar = os.path.join(relax_path, 'POSCAR_unrelaxed')
        model.astr.to(filename=new_poscar, fmt='poscar')

    def relax(self, model, reg_id):
        """
        Runs MACE relaxation using ASE.
        """
        print(f"Prepping the job folder of model {model.label}.")
        self.prep_job_folder(model, reg_id)
        ...
        # 1. Convert Pymatgen Structure (model.astr) to ASE Atoms
        atoms = AseAtomsAdaptor.get_atoms(model.astr)

        # 2. Determine Device (Worker-safe check)
        target_device = self.device
        if target_device == 'cuda' and not torch.cuda.is_available():
            print(f"Warning: CUDA requested for model {model.label} but not available. "
                  "Falling back to CPU.")
            target_device = 'cpu'

        # 3. Initialize MACE Calculator
        # We initialize this inside relax() to ensure it happens on the Worker process
        try:
            calc = mace_mp(
            ...
            # 4. Run Relaxation (BFGS)
            # Define log/traj files inside the specific calc folder
            traj_file = os.path.join(relax_path, f'mace_relax_{model.label}.traj')
            log_file = os.path.join(relax_path, f'mace_log_{model.label}.txt')
            ...
            model.tot_en = total_energy
            model.converged = opt.converged() # ASE optimizer tracks convergence

            # 6. Convert back to Pymatgen
            relaxed_astr = AseAtomsAdaptor.get_structure(atoms)
            # Normalize coords (Move atoms inside unit cell)
            self.move_atoms_inside(relaxed_astr)
            ...
            # 7. Calculate Formation Energy (Objective Function)
            comp_dict = relaxed_astr.composition.as_dict()
            astr_elems = [i.name for i in relaxed_astr.composition.elements]
            
            free_en = total_energy
            
            # Ensure we don't crash if chemical potentials are missing, but warn
            try:
                for elem in astr_elems:
                ...
                model.obj0_val = 0.0 # Default fallback or raise error

        except Exception as e:
            print(f"MACE relaxation failed for model {model.label}: {e}")
    ...
    def move_atoms_inside(self, astr):
        """
        Helper to wrap atoms into the unit cell (Fractional coords [0, 1]).
        Identical to lammps_code implementation.
        """
        species = astr.species
        fc = astr.frac_coords
        fc = np.where((fc < 0) | (fc > 1), fc - np.floor(fc), fc)
