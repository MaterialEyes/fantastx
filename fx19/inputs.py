...
def make_objects(i_dict):
    ...
    exp_sim_methods = ['PDF', 'GB_STEM', 'PRISM', 'GSASII', 'XANES', 'XRD', 'DR_PROBE'] # DR_PROBE related is being added by yuxin
    if 'exp_sim_1' in i_dict:
        if i_dict['exp_sim_1'] in exp_sim_methods:
            ...
            if method_1 == "DR_PROBE":
                Xsim1_params = get_dr_probe_params(i_dict, 'exp_sim_1_params') # get_dr_probe_params function is at bottom of inputs.py
                Xsim_1 = experimental_simulation.dr_probe_of_model(Xsim1_params) # dr_probe_of_model at bottom of experimental_simulation.py
...
def get_dr_probe_params(i_dict,exp_sim_params_id):
    """
    Reads the i_dict and returns dr_probe_params for experimental simulation method
    that is used in search (if provided). Does not mention defaults if not
    provided in input file. That happens in experimental_simulation module
