...
['gb', 'cluster', 'bulk', 'surface', 'molecule']
As an example, if looking at a `'gb'` (grain boundary) structure, the YAML information might look like:
```YAML
gb:
...
```
For a full breakdown of the inputs to provide in this section (and which additional files need to be prepared depending on the structural geometry), visit the [Geometry-specific Inputs](geometries.md) page.
...
Below is a sample YAML preparation for transmission electron microscopy, `'GB_STEM'`, including all optional inputs to exp\_sim\_params. For a detailed breakdown of the inputs to all of the forward (experiment) simulation inputs, reference the [forward (experiment) simulation inputs](forward_simulations.md) page.
